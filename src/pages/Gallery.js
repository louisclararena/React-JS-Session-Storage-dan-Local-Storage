...
class Gallery extends React.Component{
    ...
    addToCart = (selectedItem) => {
        ...
        if(localStorage.getItem("cart") !== null){
            tempChart = JSON.parse(localStorage.getItem("cart"))
            // JSON.parse() digunakan untu mengkonversi dari string to array object
        }
        // cek data yang dipilih user ke keranjang belanja
        let existItem = tempChart.find(item => item.isbn === selectedItem.isbn)

        if(existItem){
            // jika item yang dipilih ada pada keranjang belanja
            window.alert("Anda telah meilih item ini")
        } else{
            // user diminta memasukkan jumlah item yg dibeli
            let promptJumlah = window.prompt("Masukkan jumlah item beli", "")
            if(promptJumlah !== null && promptJumlah !== ""){
                // jika user memasukkan jumlah item yg di beli
                ...
                tempChart.push(selectedItem)

                // simpan array tempCart ke localStorage
                localStorage.setItem("cart", JSON.stringify(tempChart))
            }
        }
    }
